...
LOGO_PRINCIPAL = BRAND_DIR / "saphir.png"
LOGO_GODGIE = BRAND_DIR / "groupe-godgie.png"
...
st.markdown("""
<style>
.saphir-header {background:linear-gradient(135deg,#061b36,#0d653d);color:white;padding:20px;border-radius:12px;margin-bottom:16px;box-shadow:0 10px 28px rgba(6,27,54,.18);}
.saphir-header h1 {margin:0 0 4px;font-size:2rem;}
.saphir-header p {margin:0 0 6px;}
.brand-caption {color:#5f6b7a;font-size:.78rem;text-align:center;margin-top:-8px;margin-bottom:8px;}
...
def Saphir_Core_Engine(lat_parcelle, lon_parcelle, severity_ndvi, pays="Cameroun"):
    """
    ...
    distance_km = calcul_distance_km(lat_parcelle, lon_parcelle, ref[0], ref[1])
...
def simuler_analyse_sentinel2(lat, lon, culture, date_derniere_analyse=None):
    ...
    """
    rng = np.random.default_rng(int(abs(lat * lon * 100)) % 1000)
    ...
    ndvi = round(base + rng.uniform(-0.15, 0.10), 3)
    ...
    return {
        ...
        "bande_NIR": round(rng.uniform(0.3, 0.6), 3),
        "bande_RED": round(rng.uniform(0.05, 0.2), 3),
    }
...
with header_logo:
    if LOGO_PRINCIPAL.exists():
        st.image(str(LOGO_PRINCIPAL), width=120)
with header_text:
    st.markdown("""
    <div class="saphir-header">
        <h1>SAPHIR Suite v3.0</h1>
        <p>Plateforme agro-industrielle intégrée — Zone CEMAC</p>
        <small>Une solution du Groupe GODGIE S.A. | Ing. Roméo Moffo Konlack | konlack18@yahoo.com</small>
    </div>
    """, unsafe_allow_html=True)
...
if LOGO_PRINCIPAL.exists():
    st.sidebar.image(str(LOGO_PRINCIPAL), width=150)
st.sidebar.markdown("<p class='brand-caption'>Écosystème SAPHIR</p>", unsafe_allow_html=True)
...
nb_alertes = len([a for a in st.session_state["alertes_db"] if a.get("statut", "active") == "active"])
            ...
            if non_mappees:
                ...
                with st.container(border=True):
                    ...
                    **Instructions terrain :**
                    ...
                    with col_coord1:
                        borne_lat = st.number_input("Latitude de la borne (°N)", value=float(parc_active["lat"]), format="%.6f", key=f"borne_lat_{sel_map}")
                    with col_coord2:
                        borne_lon = st.number_input("Longitude de la borne (°E)", value=float(parc_active["lon"]), format="%.6f", key=f"borne_lon_{sel_map}")

                    col_m1, col_m2, col_m3 = st.columns(3)
                    with col_m1:
                        if st.button("🟢 Borne 0 — Démarrer", key=f"start_{sel_map}"):
                            st.session_state["mapping_points"][sel_map] = [{"lat": borne_lat, "lon": borne_lon}]
                            st.rerun()
                    with col_m2:
                        if st.button("📍 Enregistrer un pivot", key=f"pivot_{sel_map}"):
                            points = st.session_state["mapping_points"].get(sel_map, [])
                            if not points:
                                st.error("Définissez d’abord la borne initiale.")
                            else:
                                points.append({"lat": borne_lat, "lon": borne_lon})
                                st.session_state["mapping_points"][sel_map] = points
                                st.rerun()
                    with col_m3:
                        if st.button("🏁 Clôturer le polygone", type="primary", key=f"close_{sel_map}"):
                            points = st.session_state["mapping_points"].get(sel_map, [])
                            if len(points) < 3:
                                st.error("Un polygone nécessite au moins trois bornes.")
                            else:
                                for p in parcelles:
                    ...
                    # Visualisation du relevé réellement saisi
                    points = st.session_state["mapping_points"].get(sel_map, [])
                    if points:
                        st.map(pd.DataFrame(points), zoom=13)
                    else:
                        st.info("Ajoutez au moins une borne pour visualiser le relevé de terrain.")
        ...
        if st.session_state["parcelles_db"]:
            ...
            if st.button("🛰️ Lancer l'analyse Sentinel-2 maintenant", type="primary"):
                ...
                core = Saphir_Core_Engine(
                    ...
                    lon_parcelle=parc.get("lon", 11.91),
                ...
                for existing in st.session_state["alertes_db"]:
                    if existing.get("id_parcelle") == sel_id and existing.get("statut", "active") == "active":
                        existing["statut"] = "cloturee"
                alerte["statut"] = "active" if core["zone"] in ["ROUGE", "ORANGE"] else "cloturee"
...
# ================================================================
    ...
    **Comment ça fonctionne :**
    ...
    # Alertes actives
    alertes_actives = [a for a in st.session_state["alertes_db"] if a.get("statut", "active") == "active"]
